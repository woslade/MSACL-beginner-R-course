...
name.abbrv <- str_c(str_sub(name.split[1],1,3), 
                    str_sub(name.split[2], 1, 3))
```
...
library(scales)
library(tidyr)
K.data <- read_csv("Data_Files/Potassium.csv", sep = ",")
K.data %>%
...
clean <- function(x){
  ...
  better <- str_replace(x, "<", "")
  best   <- str_replace(better, ">", "")
...
schnitzel.finder <- function(sentence){
  sen.vec <- str_split(sentence, " ", simplify = TRUE)
  sen.vec <- str_to_lower(sen.vec) #this turns upper case SCHNITZEL to lower case schnitzel
...
```{r, include = FALSE}
###Solution###
dir_create("data_files_copy")
my_list <- dir_ls(path = "Data_Files", glob = "*.csv")
file_copy(my_list, "Data_Files_Copy")
```
...
```{r, include = FALSE}
###Solution###
old_names <- dir_ls(path = "data_files_copy", glob = "*.csv")
new_names <- str_replace(old_names, ".csv", paste("-", Sys.Date(), ".csv", sep = ""))
...
forgotten_hero <- tibble(subjects = 11, secret_identity = "Kara", hero = "Supergirl")
#you have to create it in an object of 1 column and 3 rows
write_csv(forgotten.hero, "hero_data.csv", append = TRUE)
```
...
## Option 2: Clean Up Some Research Data -- Bad Medicine
